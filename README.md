...
掌阅图书馆管理系统是一套面向读者与馆员的图书借阅与后台管理应用。主要功能包括：

- **读者端**：浏览借阅图书、按分类/关键词检索、借书与还书、个人账户（查看/修改资料、查看借阅记录）、登录与注册。
- **多语言与主题**：支持中文/英文切换，支持亮色/暗色主题。

管理员登录后可使用**管理面板**，对书籍、用户与借阅记录进行统一管理（见下文）。

---

## 管理面板功能说明

管理面板仅供管理员使用，入口为管理员登录后的顶部栏更多操作（右上角三个点）中的「管理面板」或相应路由（如 `/admin`）。管理面板的右上角同样也是支持与主站一致的主题切换与语言切换。

| 模块 | 功能说明 |
|------|----------|
| **仪表盘** | 统计卡片（书籍总数、用户数、在借数量、借阅记录数）；书籍分类占比；最近借阅列表与快捷跳转；书籍借阅数量排行榜（按当前被借阅次数 Top 10）。 |
| **书籍管理** | 书籍列表、新增书籍、编辑书籍信息、删除书籍（无在借记录时）。 |
| **用户管理** | 用户列表、搜索/筛选；支持编辑用户信息（姓名、邮箱、重置密码等）。 |
| **借阅管理** | 借阅记录列表、将指定记录标记为已还。 |
